...
$(document).ready(function(){
    ...
    $("input[name='icon']").click(function(){
        $(this).val() == "n" ? getStyle.addClass("noicon") : getStyle.removeClass("noicon");
    });
    $("input[name='layout']").click(function(){
        $(this).val() == "n" ? getStyle.addClass("vertical").find("p:first").addClass('np') : getStyle.removeClass("vertical").find("p:first").removeClass('np');
    });
    $("#showDays").change(function(){
        $("#getStyle ul li").show();
        var jsms = Number($(this).val());
        //console.log(jsms)
        var gsize = $("#getStyle ul li").size();
        for(var i = jsms;i < gsize;i++){
            $("#getStyle ul li").eq(i).hide();
        }
    });
    $("#fc").change(function(){
        var colorCode = "fc-" + colorArr[Number($(this).val())].slice(1);//提取样式
        $("#getStyle").addClass(colorCode);//重组样式
    });
    $("#bgc").change(function(){
        var colorCode = "bc-" + colorArr[Number($(this).val())].slice(1);//提取样式
        $("#getStyle").addClass(colorCode);//重组样式
    });
    $("#bdc").change(function(){
        var colorCode = colorArr[Number($(this).val())];
        if($(this).val() == ''){
            $("#getStyle").removeAttr("style");
        }else{
            var borderCol = 'border:1px solid ' + colorCode ;
            $("#getStyle").attr("style",borderCol);
        }
    });
});
var colorArr = [null,'#000','#fff','#C6C6C6','#B00C22','#D10F41','#F47837','#FBD008','#51B148','#00AFDB','#2E93D9','#005391']
